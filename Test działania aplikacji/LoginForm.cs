...
namespace Test_działania_aplikacji
{
    public partial class LoginForm : Form
    {
        ...
        private void checkCurrentUser(object sender, EventArgs e) //Sprawdzanie czy tabela z zalogowanymi użytkownikami jest pusta
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("select USER from CURRENTUSER", connection);
                connection.Open();
                ...
                while (read.Read())
                {
                    aktualnieZalogowanyUser = (read["USER"].ToString().Trim());
        ...
        private void logowanieDoSystemu(object sender, EventArgs e) //Metoda logowania usera do systemu
        {
            ...
            String sqlQuerry = "Select ROLE from LOGIN Where USER = '" + textBoxUserName.Text.Trim() + "' and PASSWORD = '" + textBoxUserPassword.Text.Trim() + "'";
            ...
            {
                MainMenuForm objMenuGlowne = new MainMenuForm();
                this.Hide();
                objMenuGlowne.Show();
                ...
                string sql = "Insert into CURRENTUSER ([USER], [ADMIN]) values(@Currentuser,@Currentadmin)";
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand(sql, connection))
                        {
                            cmd.Parameters.AddWithValue("@Currentuser", textBoxUserName.Text);
                            cmd.Parameters.AddWithValue("@Currentadmin", loggedUserAdminStatus);
        ...
        {
            if (isOldUserInDatabase == 0) //Sprawdzanie czy tabela CURRENTUSER jest pusta
            {
