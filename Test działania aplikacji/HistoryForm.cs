...
namespace Test_działania_aplikacji
{
    public partial class HistoryForm : Form
    {
        ...
        public void PopulateListView(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("select * from MAKECOATHISTORY", connection);
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                ListViewItem listitem = new ListViewItem(dr["COATQUANTITY"].ToString());
                listitem.SubItems.Add(dr["TIME"].ToString());
                listitem.SubItems.Add(dr["USER"].ToString());
                listitem.SubItems.Add(dr["COATTYPE"].ToString());
        ...
        private void DeleteRowDataBase(object sender, EventArgs e) //USUWANIE ZAZNACZONYCH DANYCH
        {
            if (listViewHistory.SelectedItems.Count > 0) //Zabezpieczenie przed brakiem wyboru wiersza z ListView
            {
                ...
                string sql = "Delete from MAKECOATHISTORY where Time=@Time AND Data=@Data";

                {
                    try
                    {
                        ...
                        {
                            cmd.Parameters.AddWithValue("@Time", godzinaTabela);
        ...
        private void usuwanie7DniTabela(object sender, EventArgs e) //NIE DZIAŁA!!!!! <====Dodaje tylko aktualną datę minus siedem
        {
            ...
            //string sql = "Delete from MAKECOATHISTORY where DATA BETWEEN '" + dataWsteczna + "' AND '" + aktualnaData + "'";
